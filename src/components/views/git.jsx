        ...
        <div className='two-columns-page'>
          ...
          <div className='half-page half'>
            ...
            <ul>
              ...
              <li><a href="https://github.com/marc921/academineHeroku/blob/master/README.md">GitHub AcadéMine: comment contribuer de manière simple, rapide et stable.</a></li>
            </ul>
          ...
          <div className='half-page half border-left'>
            ...
            <ol>
              ...
              <li>Sur votre terminal, tapez <code>git clone lien-de-clonage-ssh</code> pour cloner le dépôt distant sur votre machine. Le dossier créé est le dépôt local.</li>
              <li>Entrez dans votre dépôt local avec <code>cd nom-du-dépôt</code></li>
              <li>Listez les branches locales et la branche active avec <code>git branch</code>. Vous devez être sur la branche master. Les branches ont différents rôles, et peuvent être comprises comme des dossiers contenant chacune une version de votre projet. <i>master</i> est la branche principale, elle doit toujours contenir un code irréprochable. Ne modifiez jamais master directement si vous ne savez pas ce que vous faites. Pour modifier master, on crée des branches temporaires pour ajouter une fonctionnalité, réparer un bug ou autre. Le nom de la branche contient généralement votre nom et la fonctionnalité ajoutée. La branche sera fusionnée à master puis supprimée une fois que son objectif sera atteint.</li>
              <li>Créez une nouvelle branche à partir de la branche master avec <code>git checkout -b nom-de-ma-branche</code>. Vous êtes directement déplacé(e) dessus.</li>
              <li>Effectuez les modifications voulues dans le code du dépôt, ajoutez, renommez, supprimez des fichiers...</li>
              <li>Récapitulez les modifications avec <code>git status</code> pour voir les noms des fichiers modifiés, <code>git diff</code> pour voir les lignes modifiées dans chaque fichier.</li>
              <li>Ajouter les modifications au prochain commit avec <code>git add fichier-ou-dossier-à-ajouter</code>. Pour tout ajouter, placez-vous à la racine de votre dépôt local et tapez <code>git add .</code></li>
              <li>Réaliser le commit avec <code>git commit -m "Description des modifications réalisées"</code>. Le commit officialise les modifications et les enregistre localement. Votre branche locale revient à un état stable et on peut alors changer de branche sans danger. Ne changez pas de branche si vous avez des modifications en cours !</li>
              <li>Propagez le commit sur votre branche distante avec <code>git push origin nom-de-ma-branche</code>.</li>
              ...
              <li>Si plusieurs personnes collaborent sur votre dépôt, il est probable que la branche master ait changée depuis que vous en êtes parti(e). Vérifiez le en vous déplaçant sur la branche master de votre dépôt local avec <code>git checkout master</code> et en la mettant au niveau de la branche master du dépôt distant avec <code>git pull</code>. Il faut donc fusionner la branche master dans la vôtre pour obtenir une branche avec toutes les modifications : <code>git checkout nom-de-ma-branche; git merge master</code></li>
              ...
              <li>Vous pouvez à présent revenir sur la branche master locale avec <code>git checkout master</code>, la mettre à jour avec <code>git pull</code> et supprimer vos branches distante et locale avec <code>git branch -D nom-de-ma-branche</code></li>
